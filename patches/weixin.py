...
async def send_weixin_direct(
    *,
    ...
    # Do not reuse the live gateway aiohttp session for media sends here.
    # send_message_tool can run in a different asyncio loop/thread from the
    # gateway long-poll adapter; using that session for CDN uploads then raises
    # "Timeout context manager should be used inside a task".  Text-only sends
    # can keep the low-latency live adapter path, while media sends use the
    # one-shot session below.
    if (not media_files) and live_adapter is not None and send_session is not None and not send_session.closed:
